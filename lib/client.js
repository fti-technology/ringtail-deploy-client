...
function RingtailClient(options) {
  ...
  this.installDiagnosticTimeout = 60 * 1000;    // 60 seconds
...
RingtailClient.prototype.setConfig = function setConfig(key, value, next) {
  ...
  //debug('setting config - ' + key + ' to value' + value);
  ...
  request.get(url, function(err, response, body) {   
    if(response && response.statusCode === 200) {    
      ...
      //deferred.reject(err);
      deferred.resolve(value);
      debug('retry config value');
      me.setConfig(key, value);
    } else {
 ...
 * @return {promise}
 */
RingtailClient.prototype.waitForService = function waitForService(waitTime, next) { 
  var statusUrl = this.statusUrl
