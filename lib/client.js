 ...
 * @return {promise}
 */
RingtailClient.prototype.setMasterCredentials = function setMasterCredentials(user, pass, next) {
  var deferred = new Q.defer()    
    , url = this.setMasterConfigUrl;

    debug('updating master credentials');

    request.post(url, {form:{'MasterRunnerUser':user, 'MasterRunnerPass':pass}}, function(err, response, body) {
      if(response && response.statusCode === 200) {
        deferred.resolve(value);
      } else if(err) {
        deferred.reject(err);
