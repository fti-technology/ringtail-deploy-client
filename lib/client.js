...
RingtailClient.prototype.setMasterCredentials = function setMasterCredentials(credentials, next) {
  var deferred = new Q.defer()    
    ...
    request.post(url, {form:credentials}, function(err, response, body) {
      if(response && response.statusCode === 200) {
        deferred.resolve();
      } else if(err) {
        deferred.reject(err);
...
RingtailClient.prototype.isJobRunning = function isJobRunning() {
  ...
  var isRunningUrl = this.isRunningUrl
    ...
    request.get({url: isRunningUrl}, function(err, response, body) {
      if(response && response.statusCode === 200) {
